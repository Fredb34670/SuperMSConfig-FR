...
namespace MSNavigator
{
    public partial class MSNavigatorPluginControl : UserControl, IPlugin
    {
        ...
        public string PluginInfo => "Microsoft Navigator est votre assistant moderne pour Windows. Comme Clippy, cet outil traite des requêtes simples pour gérer votre système : installation d'applications, ajustement des paramètres, et plus encore.";
        ...
        private async Task InstallAppWithWinget(string wingetId)
        {
            try
            {
                resultsTextBox.Text = "Installation de " + wingetId;
                ...
                using (PowerShell powerShell = PowerShell.Create())
                {
                    ...
                    if (output.Any())
                    {
                        resultsTextBox.Text = "Sortie : " + string.Join(Environment.NewLine, output.Select(o => o.ToString()));
                    }

                    // Process errors
                    if (errors.Any())
                    {
                        resultsTextBox.Text += Environment.NewLine + "Erreur : " + string.Join(Environment.NewLine, errors.Select(e => e.ToString()));
                    }
                }
            }
            catch (Exception ex)
        ...
        public async Task ExecutePowerShellCommand(string command)
        {
            try
            {
                ...
                using (Process process = Process.Start(processInfo))
                {
                    ...
                    Invoke((Action)(() =>
                    {
                        resultsTextBox.Text = $"Sortie : {output}\r\nErreur : {error}";
                    }));
                }
            }
            catch (Exception ex)
        ...
        {
            searchTextBox.Text = "Tapez votre question";
